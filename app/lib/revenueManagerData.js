export const rmHeroContent = {
    tag: 'Solutions for Revenue Managers',
    title: 'Unlock Your Hotel\'s True Revenue Potential with AI Intelligence',
    description: 'Empower your pricing strategy with AI-driven intelligence, automated competitor monitoring, and demand forecasting built by revenue professionals.',
    cta: 'Book A Demo',
    videoPoster: '/images/posters/rm-hero.png',
    videoSrc: '/hero-video.mp4', // Placeholder
};

export const rmAdvantages = {
    title: 'Precision Pricing, Maximized Profit',
    subtitle: 'Spreadsheets vs. AI-Powered Dynamic Optimization',
    features: [
        {
            title: 'Dynamic Price Optimization',
            description: 'AI analyzes 100+ data points simultaneously to make continuous optimizations that are impossible to do manually.',
            icon: 'TrendingUp',
        },
        {
            title: '35% RevPAR Improvement',
            description: 'Hotels typically see a 15-35% RevPAR improvement in the first year through intelligent rate optimization.',
            icon: 'CircleDollarSign',
        },
        {
            title: 'Automated Comp Shopping',
            description: 'Continuous monitoring of competitor rates and local events ensures you never miss a demand spike.',
            icon: 'Search',
        },
    ],
};

export const rmPlatform = {
    tag: 'Revenue Intelligence',
    title: 'Advanced Revenue Management command',
    features: [
        {
            title: 'Real-Time Demand Forecast',
            description: 'Advanced algorithms predict demand patterns and manage overbooking strategies based on travel trends.',
            icon: 'LineChart',
            className: 'bg-[#eeede8] col-span-1 md:col-span-8 lg:col-span-8 min-h-[350px]'
        },
        {
            title: 'Ancillary Revenue',
            description: 'Optimize beyond room rates with intelligent package pricing, spa services, and F&B upsell opportunities.',
            icon: 'Plus',
            className: 'bg-[#dce6f2] col-span-1 md:col-span-4 lg:col-span-4 min-h-[350px]'
        },
        {
            title: 'Channel Efficiency',
            description: 'Detailed analysis of channel contribution and OTA commission impact to optimize your distribution mix.',
            icon: 'Share2',
            className: 'bg-[#eeede8] col-span-1 md:col-span-6 lg:col-span-6 min-h-[350px]'
        },
        {
            title: 'Revenue Manager Reports',
            description: 'Executive-level reporting with RevPAR analysis, ADR trends, and ROI attribution that proves your impact.',
            icon: 'BarChart3',
            className: 'bg-[#f4f1ea] col-span-1 md:col-span-6 lg:col-span-6 min-h-[350px]'
        }
    ],
    cta: {
        title: 'Ready to replace manual spreadsheets with AI-driven pricing intelligence?',
        buttonText: 'Book A Demo',
        icon: 'TrendingUp',
        className: 'bg-[#dce6f2] col-span-1 md:col-span-12 lg:col-span-12 min-h-[300px]'
    }
};

export const rmAnalytics = {
    title: 'Data-Driven Decision Making',
    description: 'Access real-time ADR, RevPAR, and booking pace analytics from one unified revenue hub.',
    image: '/image copy 13.png', // Revenue manager analytics
    lists: [
        'Automated Competitive Intelligence',
        'Market Segmentation Tools',
        'Customizable Revenue Guardrails'
    ],
    imageLeft: false,
};

export const rmFaqs = [
    {
        category: 'Intelligence',
        question: 'How does AI pricing compare to manual revenue management?',
        answer: 'Our AI analyzes hundreds of data points simultaneously and makes continuous optimizations that would be impossible manually, typically improving RevPAR by 15-20%.',
    },
    {
        category: 'Control',
        question: 'Can I override AI pricing recommendations?',
        answer: 'Absolutely. You maintain full control and can set guardrails, override specific dates, and adjust recommendations as needed.',
    },
    {
        category: 'Integration',
        question: 'Does it work with existing Revenue Management Systems?',
        answer: 'Yes, we seamlessly integrate with leading RMS providers including Duetto and IDeaS to complement your existing strategy.',
    },
    {
        category: 'Features',
        question: 'What should revenue managers look for in pricing software?',
        answer: 'Look for AI dynamic pricing, automated competitive intelligence, restriction management, and ROI attribution reporting.',
    },
    {
        category: 'Performance',
        question: 'What is the typically ROI of AI-powered revenue management?',
        answer: 'Hotels typically see a 15-25% RevPAR improvement and significant time savings by automating routine rate adjustments.',
    },
];
