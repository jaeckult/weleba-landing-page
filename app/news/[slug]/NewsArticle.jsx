...
import { newsArticles as enArticles } from '../../lib/newsData';
import { newsArticles as amArticles } from '../../lib/newsData.am';
...
export default function NewsArticle({ slug }) {
        ...
        return (
            <div className="min-h-screen flex items-center justify-center p-6">
                <div className="text-center bg-gray-50 p-12 rounded-3xl max-w-lg w-full">
                    <h1 className="text-4xl font-serif text-[#0a1628] mb-6">
                        {locale === 'am' ? 'ዜናው አልተገኘም' : 'News Not Found'}
                    </h1>
                    <p className="text-gray-600 mb-8">
                        {locale === 'am'
                            ? 'የፈለጉት ዜና ተንቀሳቅሷል ወይም ተሰርዟል ይሆናል።'
                            : "The news article you're looking for might have been moved or deleted."}
                    </p>
                    <Link href="/news" className="inline-flex items-center gap-2 bg-[#0052ff] text-white px-8 py-3 rounded-xl font-bold hover:bg-blue-700 transition-all">
                        {locale === 'am' ? 'ወደ ዜና ተመለስ' : 'Back to News'}
                    </Link>
                </div>
            </div>
        );
    ...
    const relatedNews = articles.filter(p => p.slug !== slug).slice(0, 3);
            ...
            <section className="bg-[#f8fafc] pt-32 pb-16 px-6">
                <div className="max-w-4xl mx-auto">
                    ...
                    >
                        ...
                        <span>{locale === 'am' ? 'ወደ ዜና ተመለስ' : 'Back to News'}</span>
                    </Link>
