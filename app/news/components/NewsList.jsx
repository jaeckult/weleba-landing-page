...
const NewsList = ({ articles, categories }) => {
    ...
    const filteredArticles = articles.filter(article => {
        const matchesCategory = selectedCategory === 'all' || article.category === selectedCategory || (selectedCategory !== 'all' && article.category !== selectedCategory && false); // Simplified logic: 'all' or match. The original BlogList had complex logic? No, just match.
        // Wait, BlogList logic was: selectedCategory === 'all' || post.category === selectedCategory
        // But newsCategories slug might utilize actual slugs vs names?
        // In newsData.js: { name: 'Company News', slug: 'Company News', count: 3 } - slug is same as Name usually, except 'all'.
        // Let's stick to the BlogList logic.

        const categoryMatch = selectedCategory === 'all' || article.category === selectedCategory || (categories.find(c => c.slug === selectedCategory)?.name === article.category);
    ...
    const strings = {
        en: {
            ...
            showing: 'Showing',
            records: filteredArticles.length === 1 ? 'article' : 'articles',
            readMore: 'Read More',
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="mb-8">
                    <p className="text-gray-600">
                        {strings.showing} <span className="font-semibold text-gray-900">{filteredArticles.length}</span> {strings.records}
                    </p>
                </div>
